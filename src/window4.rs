...
impl Window4 {
    ...
    pub fn close(&mut self, window: &MainWindow) {
        self.active = false;
        window.set_window_4_active(false);
    }

    pub fn toggle(&mut self, window: &MainWindow) {
        self.active = !self.active;
        window.set_window_4_active(self.active);
    }

    pub fn translate(&mut self, window: &MainWindow, x: i32, y: i32) {
        self.x_translation = x;
        self.y_translation = y;
        window.set_window_4_x_translation(x);
        window.set_window_4_y_translation(y);
    }

    pub fn scale(&mut self, window: &MainWindow, x: i32, y: i32) {
        self.x_scale = x;
        self.y_scale = y;
        window.set_window_4_x_scale(x);
        window.set_window_4_y_scale(y);
    }
